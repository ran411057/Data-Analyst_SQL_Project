...
select artist.artist_id, artist.name,count(artist.artist_id) as songs_sung from track
join album on album.album_id = track.album_id
join artist on artist.artist_id = album.artist_id
join genre on genre.genre_id = track.genre_id
where genre.name like 'Rock'
group by artist.artist_id
order by songs_sung desc
limit 10
...
CTE= Common temporary expressions, it helps in creation temoprary table, in reallity it does not exist.
